package com.ray.library.imgseletor;
...
import com.ray.library.imgseletor.adapter.FolderAdapter;
import com.ray.library.imgseletor.adapter.ImageGridAdapter;
import com.ray.library.imgseletor.bean.Folder;
import com.ray.library.imgseletor.bean.Image;
import com.ray.library.imgseletor.utils.FileUtils;
import com.ray.library.imgseletor.utils.ScreenUtils;
...
public class MultiImageSelectorFragment extends Fragment {
    ...
    /** Original data set */
    public static final String EXTAR_ONLY_CAMERA = "only_camera";
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        if(getArguments().getBoolean(EXTAR_ONLY_CAMERA)){//只是拍照 直接进入相机
            showCameraAction();
        }
            ...
            if (intent.resolveActivity(getActivity().getPackageManager()) != null) {
                ...
                if (mTmpFile != null && mTmpFile.exists()) {
                    if(Build.VERSION.SDK_INT >= 24){
                        ...
                        intent.putExtra(MediaStore.EXTRA_OUTPUT, FileProvider.getUriForFile(getActivity(), getActivity().getPackageName() + ".fileprovider",mTmpFile));
                    }else
    ...
    private void selectImageFromGrid(Image image, int mode) {
        if(image != null) {
            if(image.imageView!=null&&mImageAdapter.getErrorDrawable()!=null&&image.imageView.getDrawable().equals(mImageAdapter.getErrorDrawable())){
                Toast.makeText(getContext(),"该图片已损坏，请重新选择", Toast.LENGTH_SHORT).show();
                return;
            }
            if(mode == MODE_MULTI) {
