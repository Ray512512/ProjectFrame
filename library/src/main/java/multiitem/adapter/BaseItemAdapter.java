...
import multiitem.adapter.holder.HeadFootHolderManager;
import multiitem.adapter.holder.ViewHolderManager;
import multiitem.adapter.holder.ViewHolderManagerGroup;
import multiitem.adapter.holder.ViewHolderParams;
import multiitem.adapter.type.ItemTypeManager;
import multiitem.animation.AnimationLoader;
import multiitem.animation.BaseAnimation;
import multiitem.common.Const;
import multiitem.item.LoadMoreManager;
import multiitem.item.UniqueItemManager;
import multiitem.listener.OnItemClickListener;
import multiitem.listener.OnItemLongClickListener;
...
public class BaseItemAdapter extends RecyclerView.Adapter<BaseViewHolder> {
    ...
    public BaseItemAdapter(DiffUtil.Callback  d){
        diffCallback=d;
        itemTypeManager = new ItemTypeManager();
    }
    /**
    ...
    public void setDataItem(int position, @NonNull Object item) {
        dataItems.set(position,item);
        notifyItemRangeChanged(position + getHeadCount(), 1);
    }
    public void setDataItemByDiff(int position, @NonNull Object item) {
        dataItems.set(position,item);
  ...
  /*  public void setItemData(int position, @NonNull Object item) {
        dataItems.set(position,item);
//        notifyItemRangeChanged(position + getHeadCount(), 1);
    }*/
    /**
    ...
    public boolean isAnim=false,isShowAnim=false;
    @Override
...
//        System.out.println("onViewAttachedToWindow:::" + holder.getItemPosition() + "==" + holder.getItemData());
        ...
        if(isAnim){
            ...
            new Handler().postDelayed(() -> isAnim=isShowAnim, 1000);
        }
    }
